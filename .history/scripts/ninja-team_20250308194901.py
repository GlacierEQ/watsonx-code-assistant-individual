...
class NinjaTeamOrchestrator:
    ...
    def parse_build_tasks(self, build_dir):
        ...
        # Handle missing build.ninja or existing one with issues
        if not os.path.exists(ninja_file) or os.path.getsize(ninja_file) == 0:
            logger.error(f"Ninja build file not found at {ninja_file}")
            self._create_minimal_build_file(build_dir)
        
        # Try to parse the build file
        try:
            # Use the wrapper script in WSL if available, otherwise use direct command
            wsl_available = False
            if sys.platform == "win32":
                try:
                    # Check if WSL is available
                    subprocess.run(['wsl', 'ls'], stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=True)
                    wsl_available = True
                except:
                    wsl_available = False
            
            if wsl_available and os.path.exists("scripts/ninja-wsl-wrapper.sh"):
                # Use the WSL wrapper for Ninja commands
                logger.info("Using WSL wrapper for Ninja commands")
                cmd = f"wsl scripts/ninja-wsl-wrapper.sh {build_dir} targets all"
                output = subprocess.check_output(cmd, shell=True).decode()
            else:
                # Use direct Ninja command
                logger.info("Using direct Ninja command")
            ...
            logger.info("Creating a minimal build file for basic operation")
            self._create_minimal_build_file(build_dir)
            return self._try_minimal_build(build_dir)
        
        except Exception as e:
            logger.error(f"Error parsing build tasks: {e}")
            return False

    def _create_minimal_build_file(self, build_dir):
        ...
        with open(ninja_file, "w") as f:
            ...
            f.write("build placeholder: touch\n")
            f.write("  pool = console\n\n")
            f.write("default placeholder\n")
        
        logger.info(f"Created minimal build.ninja file at {ninja_file}")

    def _try_minimal_build(self, build_dir):
        """Try to parse a minimal build file instead"""
        try:
            task = BuildTask(
                target="placeholder", 
                directory=build_dir,
            ...
            self.build_tasks = [task]
            self.build_graph["placeholder"] = []
            
            logger.info("Created minimal build task for basic operation")
            return True
        except Exception as e:
            logger.error(f"Failed to create minimal build: {e}")
            return False
